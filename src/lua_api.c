...
static int play_music_file(const char *name) {
    char path[1024];
...
static int lua_sfx_music(lua_State *L) {
    ...
    ensure_audio();
    if (!IsAudioDeviceReady()) {
        strncpy(pending_music, name, sizeof(pending_music) - 1);
        pending_music[sizeof(pending_music) - 1] = 0;
        return 0;
    }
    play_music_file(name);
    return 0;
}
...
void lua_api_audio_update(void) {
    if (pending_music[0]) {
        ensure_audio();
        if (IsAudioDeviceReady()) {
            play_music_file(pending_music);
            pending_music[0] = 0;
        }
    }
    if (music_loaded && IsAudioDeviceReady()) UpdateMusicStream(current_music);
}
